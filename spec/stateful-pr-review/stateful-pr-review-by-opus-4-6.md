...
Each inline comment posted on a specific code line follows this format:

```markdown
**{skill} > {rule}**

{Description of the violation — references the specific code construct, explains why it violates
the rule, identifies the impact (e.g., defeats type checking, allows silent failures), and states
why the current code has no reason to deviate from the rule.}

**Suggestion:** {Concrete fix — names the exact change (e.g., "Change `product: any` to
`product: Product`"), references existing types/imports already available, and notes the
minimal effort required (e.g., "single-word change").}

```{language}
{Corrected code block showing the fix applied}
```
...
> **nextjs-react-best-practices > Cat 5: TypeScript Strictness**
>
> The `ProductCardProps` interface declares `product: any` on line 7, despite the `Product` type being imported on line 3 from `@/features/products/types/product`. This completely defeats TypeScript's compile-time type checking for every property access on `product` throughout the component (lines 22, 23, 31, 32, 33, 38). Any typo such as `product.titel` instead of `product.title` would silently pass compilation and only fail at runtime. The `Product` interface is well-defined with `id`, `name`, `description`, `price`, and `image` fields, so there is no reason to use `any` here.
>
> **Suggestion:** Change `product: any` to `product: Product` in the `ProductCardProps` interface. The `Product` type is already imported on line 3, so the fix is a single-word change:
>
> ```tsx
