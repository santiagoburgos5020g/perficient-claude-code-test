# SOLID Principles Reference — Skill Specification (Opus 4.6 Reviewed)
...
The 5 SOLID principles work together as a cohesive set of guidelines for writing maintainable, flexible, and robust object-oriented code. They are not independent rules to apply in isolation — they reinforce and depend on each other:
...
Agents should consider these interactions when multiple principles appear relevant to the same code.
...
**Patterns That Help Enforce SRP:** Facade (hides subsystem complexity behind a focused interface), Mediator (extracts coordination logic out of components), Command (extracts operations into dedicated objects), Strategy (extracts algorithm variations into separate classes)
...
**Patterns That Help Enforce OCP:** Strategy (swap algorithms without modifying context), Template Method (vary steps without changing skeleton), Decorator (add behavior without modifying the base), Observer (add subscribers without modifying the publisher), Factory Method (add new product types without changing client code)
...
4. **History constraint** — A subclass must not introduce state changes that the parent would not allow (no new mutable fields that break assumptions about the object's history)
...
**Patterns That Help Enforce LSP:** Template Method (defines a contract that subclasses extend without breaking), Strategy (replaces inheritance with composition when subclasses diverge too much), Adapter (wraps an incompatible class instead of forcing it into a hierarchy)
...
**Patterns That Help Enforce ISP:** Adapter (adapts a fat interface to a narrow one for a specific client), Facade (presents a simplified interface to a complex subsystem), Proxy (controls access through a narrow interface)
...
**Patterns That Help Enforce DIP:** Abstract Factory (creates families of objects through abstractions), Strategy (injects algorithm implementations), Bridge (separates abstraction from implementation), Observer (decouples publisher from subscriber through an abstract subscription interface)
...
When agents identify SOLID violations during a code review or modification, they should follow these guidelines for refactoring:
...
### When Multiple Principles Are Violated

If a single piece of code violates multiple SOLID principles, this is the recommended order of resolution:
...
This order works because each step creates the preconditions for the next.
