...
2. **Check for code smells** — Are there indicators that a pattern is needed? (see per-pattern "Code Smells" sections)
3. **Match to a pattern** — Consult the "When to Use" section of candidate patterns; use the Quick Reference table to narrow candidates
...
All patterns are organized by intent:
